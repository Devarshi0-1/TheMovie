...
> ▸ **Current focus:** **Live E2E verification** — the whole backend (Phases 0–6) is built & merged but only verified offline. The immediate next step is exercising it against live Postgres+pgvector / Redis / OpenAI / TMDB. **Follow [`backend/VERIFICATION.md`](./VERIFICATION.md)** (env setup → `drizzle-kit migrate` → `bun run ingest` → per-feature checks) and tick the "Verification debt" boxes below as each passes. _After_ that, **Phase 7.1** — Frontend scaffold (Vite+ workspace with oxlint/oxfmt/Vitest/tsgo v7, TanStack Start + React 19, and `packages/schemas/` lifting the backend's Zod schemas into a shared package). _(✅ Phase 0–5 · ✅ Phase 6 — security, cost/observability, Docker + backend CI. Pending: HITL confirmation UI (Phase 7.3); a unit-test isolation flake — see VERIFICATION.md "Known issue".)_ Update this pointer as phases complete so a fresh session knows where to start (see `CLAUDE.md` → "Working cadence & context hygiene")._

> ⚠️ **Verification debt — pending live env.** These were built and verified **offline** (schema, generated SQL, `tsc`, mocked unit tests) under autonomous mode B. **Step-by-step runbook with commands: [`backend/VERIFICATION.md`](./VERIFICATION.md).** Exercise them against a live **Postgres+pgvector**, **Redis**, **`OPENAI_API_KEY`**, and **`TMDB_READ_ACCESS_API_KEY`**, then tick:
