...
## 🚩 Phase 10: TV shows as first-class

Until now TV was a deliberate **TMDB-proxy** second-class citizen — browse/detail pages reusing the movie UI, but **no ingestion, no embeddings, no semantic search, no summaries, no watchlist, and invisible to the chat agent** (movies-only retrieval, intent gate, and system prompt). This phase makes **TV a fully-embedded, first-class media type at parity with movies** (per the product decision to fully embed TV, not keep it a proxy). Shipped as **4 phased PRs**, sequenced by dependency — the data foundation lands first because hybrid suggest, semantic search, summaries, and the agent's semantic tools all depend on TV being in the DB.

**Architecture:** a **separate `tv_shows` table** mirrors `movies` 1:1 (rather than a big-bang unified `media` refactor of the verified movie pipeline); the two cross-cutting **user-owned** tables (`watchlist`, `review`) instead get a `mediaType` discriminator. TMDB's `name`/`first_air_date` normalize to `title`/`releaseDate` at ingest, so the shared `MovieResult` shape and all downstream catalog code apply to shows unchanged.

### Milestone 10.1: TV data foundation  _(complete; live-verified on the docker `themovie` catalog)_
* [x] **`tv_shows` table** (`src/db/schema.ts`, migration `0010_add_tv_shows.sql`) — a 1:1 mirror of `movies`: `tmdb_id` (unique), title/overview/poster/backdrop/release_date, genres/keywords/metadata jsonb, the plot `embedding vector(1536)` + reception `review_summary_embedding vector(1536)`, the source/summary hashes + refresh-clock columns, and the **same four indexes** (genres GIN, two HNSW cosine indexes, the `review_summary_at` btree).
* [x] **TV ingestion job** (`src/jobs/ingest-tv.ts`, `bun run ingest:tv [--incremental] [--pages=N]`) — the TV mirror of `ingest.ts`: pulls `/discover/tv` (backfill) or `/tv/popular` (incremental), enriches each show (detail + keywords via `append_to_response`), maps the TV shape (`name`→title, `first_air_date`→releaseDate, keywords under `keywords.results`), then runs the **same idempotent core** (prepare → skip-unchanged-by-`source_hash` → embed only the changed subset → upsert keyed on `tmdb_id`). Reuses the shared embedding compose/hash/cap/cache helpers, so a show and a film land in **one vector space** and re-runs are cheap no-ops. New TMDB fetchers `discoverTvPage`/`getPopularTvPage`/`getTvForIngest` in `src/lib/tmdb.ts`.
* [x] **TV retrieval tiers** (`src/agent/retrieval-tv.ts`) — `searchTvSql` (title ILIKE / year / genre jsonb-membership), `semanticSearchTv` (plot / reception / **both** via the reused RRF fusion), `fetchTvFromTmdb` (last-resort lookup + self-healing write-back), and `getTrendingTvShows`, all over `tv_shows` with injectable deps. The generic RRF fusion + row mapper are reused from the movie module (exported), so only the table differs.
* [x] **Tests** (`src/jobs/ingest-tv.test.ts`, `src/agent/retrieval-tv.test.ts`) — TV-field mapping, idempotency (skip/re-embed/mixed-batch/dedupe/invalid/empty/bad-dim), and the retrieval tiers (filters, plot/both kNN, RRF, write-back self-heal, 0-placeholder regression). **Backend suite 319 green** (was 206); tsgo + oxlint + Prettier clean.
* [x] **Live-verified** against the docker `themovie` DB: `ingest:tv --pages=2` embedded 37 shows; a re-run skipped 17/20 (idempotent); semantic search returns sensible shows ("drug empire"→crime dramas, "sitcom"→Family Guy/Simpsons/Rick and Morty).

### Milestone 10.2: Search parity — hybrid + autocomplete across movies + TV  _(complete)_
* [x] **`/api/v1/tv/suggest`** — hybrid local (`tv_shows` ILIKE) + TMDB typeahead, mirroring `/movies/suggest` (`suggestTvShows` in `lib/suggest.ts`; every hit carries `mediaType: 'tv'` so cards route to `/tv/:id`).
* [x] **Multi-suggest** endpoint — `GET /api/v1/search/suggest` (`routes/search.ts`, `suggestAll`) returns `{ movies, tv }` (shared `GroupedSuggestionsSchema`), each group independently blended/deduped/capped and degrading to `[]` on failure without affecting the other.
* [x] **Frontend**: `SearchBox` (now `scope: 'all' | 'tv'`) + `CommandSearch` (⌘K) render **Movies + TV Shows** groups via `suggestAllQueryOptions`, routing each pick by `mediaType`; the TV browse page reuses `SearchBox scope="tv"` (`suggestTvQueryOptions`) for real typeahead. Suites: backend **330** + schemas **58** + frontend **174**, all green; tsgo + oxlint + `tsc` + Prettier clean.

### Milestone 10.3: Watchlist + reception summary for TV  _(complete; live-verified)_
* [x] **`watchlist` + `review` `mediaType` migration** ('movie' | 'tv') — migration `0011`: `media_type` column (default 'movie', backfills existing rows), unique constraints swapped to (user, mediaType, movieId) (`unique_user_media`, `unique_user_media_review`), `review_media_idx` on (mediaType, movieId). Libs/routes thread `mediaType`: watchlist membership set is versioned (`watchlist:v2:…`) with `mediaType:id` members; review recent cache keys to `{mediaType}:{id}:reviews:recent` (movie keys unchanged). REST: `mediaType` in the add body + as a `?mediaType` query on DELETE/status; recent-reviews GET generalized to `/reviews/:mediaType/:id`. Frontend watchlist lib/hooks/button/page + chat HITL all carry `mediaType` (status query key is per-media). Recommendations seed from **movie** entries only (TV-seeded recs are 10.4).
* [x] **TV detail**: watchlist button (`mediaType="tv"`) + **"What audiences say"** — `GET /api/v1/tv/:id/summary` reuses the summary pipeline parametrized by media type (`summaryDeps('tv')` → `tv_shows` table, `getTvReviewMeta`, `tv:{id}:summary` cache), durable + embedded into `tv_shows.review_summary_embedding` like movies. Live-verified: a catalog show's summary generated, persisted (`review_count_at_summary`, reception vector, summary jsonb all set), and served from cache on re-read; a non-catalog show is still summarized + Redis-cached (0-row best-effort UPDATE). Suites: backend **340** / schemas **63** / frontend **180**, all green; tsc + tsgo + oxlint + Prettier clean.

### Milestone 10.4: Chat agent TV parity  _(complete)_
* [x] **TV-aware retrieval tools** (+ `mediaType` on `manage_watchlist`), wired into the agent loop. Added a parallel TV toolset over the `tv_shows` catalog — `search_tv_sql`, `semantic_search_tv`, `fetch_tv_from_tmdb` (tiered cheapest-first, mirroring the movie tiers), `get_trending_tv`, and `summarize_tv_reviews` (reuses `summaryDeps('tv')`) — leaving the verified movie tools untouched. A small `asTvResults` helper stamps every TV hit `mediaType: 'tv'` so the chat result cards route to `/tv/:id` (fixed `ChatMovieResults`, which hard-coded `/movie/$id`). `ManageWatchlistMovieSchema` gained a per-item `mediaType` (default `'movie'`), threaded through the agent prompt → HITL `WatchlistConfirm` (applies each item against the right media's REST endpoint). Friendly TV labels added to the activity trail.
* [x] **Intent gate + system prompt** updated to treat TV queries as on-topic (intent-gate prompt + `IntentResultSchema` descriptions now say "movies or TV shows"; agent system prompt now lists the TV tools and the `mediaType` rule); **recommendations** seed from TV watchlist entries too — `recommendForUser` is now media-aware (each seed finds neighbours within its own catalog, candidates deduped by `(mediaType, tmdbId)`, per-media watched-exclusion, `RecommendationSchema` gained `mediaType`). Suites: backend **348** / schemas **65** / frontend **184**, all green; tsgo + oxlint + Prettier clean.

> Every change here ships feature + edge-case tests + a UX overview, per `CLAUDE.md`.
