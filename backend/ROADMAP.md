...
> ▸ **Current focus:** **Phase 10 — TV shows as first-class** (full embedding parity, 4 phased PRs). **10.1 (TV data foundation)** and **10.2 (search parity)** are complete. **10.2** shipped TV typeahead at parity with movies: `GET /api/v1/tv/suggest` (`suggestTvShows`) + a grouped multi-suggest `GET /api/v1/search/suggest` (`suggestAll` → `{ movies, tv }`, shared `GroupedSuggestionsSchema`), and a frontend where `SearchBox` (new `scope: 'all' | 'tv'`) + `CommandSearch` (⌘K) render **Movies + TV Shows** groups and route each pick by `mediaType`; the TV browse page reuses `SearchBox scope="tv"`. Suites: backend **330** / schemas **58** / frontend **174**, all green; tsgo + oxlint + `tsc` + Prettier clean. **Next: Milestone 10.3 (watchlist + reception summary for TV — `mediaType` discriminator on `watchlist`/`review`, TV detail watchlist button + "What audiences say").** _(Phases 0–9 below remain complete.)_
>
...
### Milestone 10.3: Watchlist + reception summary for TV
* [ ] **`watchlist` + `review` `mediaType` migration** ('movie' | 'tv'); update schema/lib/routes/Redis keys + the frontend.
* [ ] **TV detail**: add the watchlist button + **"What audiences say"** (`GET /api/v1/tv/:id/summary`, durable + embedded into `tv_shows.review_summary_embedding` like movies).
