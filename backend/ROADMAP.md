...
> ▸ **Current focus:** **Phase 10 — TV shows as first-class** (full embedding parity, 4 phased PRs). **10.1 (TV data foundation) is complete & live-verified** — `tv_shows` table + migration `0010`, `ingest-tv.ts` job (`bun run ingest:tv`), and `retrieval-tv.ts` (SQL + plot/reception/both semantic search), all mirroring the movie pipeline; backend suite **319 green**. **Next: Milestone 10.2 (search parity — `/tv/suggest` + multi-suggest + Movies/TV autocomplete groups).** _(Phases 0–9 below remain complete.)_
>
...
### Milestone 10.2: Search parity — hybrid + autocomplete across movies + TV
* [ ] **`/api/v1/tv/suggest`** — hybrid local (`tv_shows` ILIKE) + TMDB typeahead, mirroring `/movies/suggest`.
* [ ] **Multi-suggest** endpoint (movies + TV in one call) so the discover + navbar search return both, grouped.
* [ ] **Frontend**: `SearchBox` + `CommandSearch` (⌘K) show **Movies + TV Shows** groups with autocomplete; the TV browse page gets real typeahead (today it's a plain input).
