...
> ▸ **Current focus:** **Phase 7.3 — Chat window (headline UX).** ✅ Phase 7.2 fully done: all four core screens ship in one PR — **Auth** (`/signin` + `/signup` on a shared-Zod `AuthForm`, session-aware nav + client `RequireAuth` guard), **Discovery** (`/` SSR-prefetches trending, `?q=` client search), **Movie detail** (`/movie/$id` SSRs details + backdrop/runtime/rating, client-loaded spoiler-free AI summary, `WatchlistButton`), and **Watchlist** (`/watchlist`, protected, remove + cache-synced). The data layer (`lib/api.ts` `apiFetch` with `credentials:'include'`, `lib/tmdb.ts` raw-TMDB→`MovieResult` mapping incl. a static genre map, `lib/auth.ts`, `lib/watchlist.ts`, live movie queries in `lib/movies.ts`) validates every boundary with the shared schemas. Dev server moved to **:5173** (matches backend `FRONTEND_URL`/CORS; backend runs on **:3100** locally — `VITE_API_URL` is the knob, default `http://localhost:3100`). **Live-verified against the running backend (:3100):** discovery/detail SSR render real TMDB data; the full auth+watchlist contract (sign-up → cookie → get-session → add/list/status/remove) round-trips cross-origin. Frontend **79** tests (was 14) + backend 159 + schemas 42 all green; oxlint + tsgo clean; `vite build` builds client + SSR. **Next: Phase 7.3** — chat window on `useChat` (`POST /api/v1/chat`, streaming + tool/retrieval activity + stop/regenerate), the **HITL tool-confirmation UI** for chat-driven watchlist mutations (approve/deny → `addToolResult`), and TanStack Form + Zod forms handling the intent-gate refusal gracefully. _Phase 7.1 recap: Bun workspace + `@themovie/schemas` + scaffolded TanStack Start app (Vite 8 / React 19 / SSR-query via `setupRouterSsrQueryIntegration`), toolchain oxlint/oxfmt/Vitest/tsgo v7._ **Live E2E verification is ✅ done** — the whole backend (Phases 0–6) was exercised against live Postgres+pgvector / Redis / OpenAI / TMDB per [`backend/VERIFICATION.md`](./VERIFICATION.md); all "Verification debt" boxes below are ticked. The live run surfaced and fixed **six bugs that only manifest against real services**: jsonb double-encoding (genres/keywords/metadata stored as string scalars → SQL `@>`/GIN filters silently matched nothing), Bun.serve's 10s `idleTimeout` killing the streaming chat agent mid-answer, `fetch_from_tmdb` treating a `tmdbId: 0` placeholder as a real id, the agent exhausting its step budget on heavy fan-out and streaming an empty answer, the review recent-list cache keeping a stale duplicate after an edit, and the Redis client never reconnecting after an outage. _(✅ Phase 0–6 + 7.1 + 7.2. Pending: chat window + HITL confirmation UI (Phase 7.3).)_ Update this pointer as phases complete so a fresh session knows where to start (see `CLAUDE.md` → "Working cadence & context hygiene")._
...
> - [x] **Phase 4.1 intent gate** — `gpt-5-mini` allows movie queries; blocks off-topic + injection with a streamed refusal and **0 gpt-5 calls**; prompt-cache reads register in the usage log.
...
> - [x] **Phase 4.3 chat endpoint** — gpt-5 streams a synthesized answer; tool/retrieval activity surfaces in the stream; blocked query streams the refusal; `onFinish` logs `retrieval=…` paths + token usage (with `cached=` prompt-cache hits). _(Fixed: Bun.serve 10s idle-timeout cutting the stream; empty-answer on step-budget exhaustion.)_
...
> - [x] **Phase 5.2 reviews + recs** — review upsert + per-movie list + cache cold-hydrate (fixed a stale-duplicate-on-edit cache bug); `GET /api/v1/recommendations` returns "because you watched X" picks via `gpt-5-mini` over pgvector candidates; empty watchlist → `[]` with no model call.
...
* [x] **Shared Zod schema** `IntentResultSchema` + `INTENTS` + pure `isBlocked`/`refusalFor`/`decideGate`. _Now lifted to `packages/schemas/` (`@themovie/schemas`, Phase 7.1); the backend imports it from there._ Verified offline (`intent.test.ts` in the schemas package + `src/agent/intent.test.ts`); live `gpt-5-mini` call confirmed in the E2E pass.
