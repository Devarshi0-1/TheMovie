...
> ▸ **Current focus:** **Phase 7.2 — Core screens.** ✅ Phase 7.1 fully done: the repo is a **Bun workspace** with **`@themovie/schemas`** (shared Zod) and a scaffolded **`frontend/`** app — Vite 8 + React 19 + **TanStack Start** (file-based Router under `src/routes/`, SSR) + **TanStack Query** (wired via `setupRouterSsrQueryIntegration`, the current SSR-query bridge), toolchain **oxlint/oxfmt/Vitest/tsgo v7** (versions pinned via the root lockfile), consuming `@themovie/schemas` end-to-end — the `/` route SSRs schema-validated movies through a real Query loader (`ensureQueryData` → `useSuspenseQuery` → dehydration). Frontend CI job is live (oxlint + tsgo + vitest); backend 159 + schemas 42 + frontend 14 tests all green; `vite build` builds client + SSR. **Next: Phase 7.2 core screens** — Auth UI (BetterAuth, session-aware routing), Discovery (trending grid + search via TanStack Query against the movie endpoints), Movie detail (+ spoiler-free AI summary, add/remove watchlist), and the Watchlist screen. Then 7.3 (chat window on `useChat` + HITL confirmation UI). **Live E2E verification is ✅ done** — the whole backend (Phases 0–6) was exercised against live Postgres+pgvector / Redis / OpenAI / TMDB per [`backend/VERIFICATION.md`](./VERIFICATION.md); all "Verification debt" boxes below are ticked. The live run surfaced and fixed **six bugs that only manifest against real services**: jsonb double-encoding (genres/keywords/metadata stored as string scalars → SQL `@>`/GIN filters silently matched nothing), Bun.serve's 10s `idleTimeout` killing the streaming chat agent mid-answer, `fetch_from_tmdb` treating a `tmdbId: 0` placeholder as a real id, the agent exhausting its step budget on heavy fan-out and streaming an empty answer, the review recent-list cache keeping a stale duplicate after an edit, and the Redis client never reconnecting after an outage. _(✅ Phase 0–6 — incl. live verification. Pending: HITL confirmation UI (Phase 7.3).)_ Update this pointer as phases complete so a fresh session knows where to start (see `CLAUDE.md` → "Working cadence & context hygiene")._
